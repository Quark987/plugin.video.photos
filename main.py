...
addon = xbmcaddon.Addon('plugin.video.photos')
photos = SynologyPhotos()
...
def list_categories():
    ...
    categories = photos.get_categories()
...
def list_albums(category, keyword=None):
    ...
    albums = photos.get_albums(category, keyword)
...
def list_photos(list_id, keyword=None, passphrase=None):        # Keyword for searches, passphrase for shared_with_me albums
    ...
    xbmcplugin.setContent(_handle, 'images')
    photolist = photos.get_photos(list_id, keyword, passphrase)

    for k in range(len(photolist)):
        list_item = xbmcgui.ListItem(label=photolist[k]['filename'])

        list_item.setArt({'thumb': photolist[k]['url'],
                          'icon': photolist[k]['url'],
                          'fanart': photolist[k]['url']})
        ...
        if photolist[k]['type'] == 'video':
            ...
            video_id = str(photolist[k]['additional']['thumbnail']['unit_id'])
            ...
            url = photos.get_photo_url(list_id, photo_id, photo_cache_key, passphrase)

            list_item.setMimeType('image/'+photolist[k]['filename'].split('.')[-1])    # Predefine the mime type, otherwise it takes ages
...
if __name__ == '__main__':
    ...
    if response == 'success':
        ...
        xbmcgui.Dialog().ok('Synology photos', response)
